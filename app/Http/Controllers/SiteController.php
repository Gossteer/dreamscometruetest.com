...
class SiteController extends Controller
{
    public function packages()
    {
        ...
        return view('site.packages', ['Carbon' => $Carbon_add14, 'Cardon_hot' => $Carbon_now, 
            'tours_hots' => Tour::whereRaw('Start_Date_Tours <= ? and Start_Date_Tours >= ? and Confidentiality = 0',[$Carbon_add14, $Carbon_now])->orderBy('Start_Date_Tours')->paginate(4),
    ...
    public function adminindex()
    {
        ...
        // }
        $count_tour_not_complit = tour::whereRaw('Confirmation_Tours = 0 and LogicalDelete = 0 or Start_Date_Tours >= ?',[$today_tour])->get();
        $count_men_customers = Customer::where('Floor', 0)->count();
        $count_women_customers = Customer::where('Floor', 1)->count();

        return view('admin.index', [
        'last_tours' => Tour::whereRaw('Confirmation_Tours = 1 and LogicalDelete = 0 and Start_Date_Tours <= ?',[$today_tour])->orderByRaw('Expenses + Profit DESC, Start_Date_Tours DESC')->paginate(3),
        ...
        'count_tour_not_complit' => count($count_tour_not_complit), 'tours_not_complit' => $count_tour_not_complit, 'today_tour' => $today_tour,
